...
class my_evaluation:
    ...
    def confusion(self):
        ...
        df_confusion = pd.crosstab(self.actuals, self.predictions)
        ...
        for label in self.classes_:
            ...
            for i in range(len(self.actuals)):
                if(self.actuals[i] == label and self.actuals[i] == self.predictions[i]):
                    tp = tp +1
                elif(self.actuals[i] == label and self.actuals[i] != self.predictions[i]):
                    fn = fn + 1

            for i in range(len(self.actuals)):
                ##print("i: " + str(i))
                ##print("actual: " + str(self.actuals[i]))
                ##print("self.predictions: " + str(self.predictions[i]))
                ##print()
                if(self.actuals[i] != label and self.actuals[i] == self.predictions[i]):
                    tn = tn +1
        ...
        if target:
            ...
            if average == "micro":
                ...
                for label in self.classes_:
                    ...
                    f1_score_fin =  f1_score_fin + f1_score * ratio
                    #print("ratio in: " + str(ratio))
                    #print("f1_score")
                    #print(f1_score_fin)
                #print("returning f1: " + str(f1_score_fin))
                f1_score = f1_score_fin
